...
async function handleUserInput(evt) {
    ...
    try {
        const response = await fetch('/api/draftLog', reqOptions);
...
function displayGreetings() {
    const draftGreeting = document.getElementById('draftGreeting');
    for (let i = 0; i < text.length; i++) {
        setTimeout(() => {
            draftGreeting.textContent += text.charAt(i);
        }, i * 50);
    }
}
...
async function main() {
    ...
    document.getElementById('draftUserinput').disabled = true;
    // display greetings
    await displayStringAI('Hello and thank you for visiting our AI program. Please enter your question, or scan the QR code to get more information.');
    await displayImgAI('/img/iChongqingQRCode.jpg');
